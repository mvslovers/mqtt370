...
#if 1   /* new code */
int mqtc_processing_thread(void *arg1, void *arg2)
{
    int         rc          = 0;
    MQTC        *mqtc       = (MQTC*)arg1;
	...
	wtof("MQTC0061I STARTING MQTT Client      TCB(%06X) TASK(%06X) STACKSIZE(%u)",
...
quit:
	wtof("MQTC0060I SHUTDOWN MQTT Client      TCB(%06X) TASK(%06X) STACKSIZE(%u)",
		tcb, task, task->stacksize);
...
int mqtc_processing_thread(void *arg1, void *arg2)
{
    ...
    while(mqtc) {
        ...
        if (mqtc->flags & MQTC_FLAG_ERROR) {
            ...
            /* the client socket will change,
             * so we need to loop back to the top of this loop
             */
            continue;
        }
