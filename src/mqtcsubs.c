...
int mqtc_sub(MQTC *mqtc, unsigned qos, const char *topic)
{
    int             rc      = 4;
    unsigned        topiclen;
    unsigned        remaining;
    ...
    remaining = sizeof(msgid) + 2 + topiclen + 1;
    ...
    pos = 0;
    pb->buf[pos++] = TYPE_SUBSCRIBE | 0x02; /* QOS 1 */
